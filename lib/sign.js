...
// CRITICAL: Create encoder that doesn't add tag 64 for Uint8Array or tag 259 for Maps
// In React Native, Buffer is a polyfill that extends Uint8Array
// Without this, cbor-x adds tags that cause toBeSigned mismatch between client and server
const cborEncoder = new cbor.Encoder({ tagUint8Array: false });
...
async function doSign (SigStructure, signer, alg) {
  ...
  const ToBeSigned = cborEncoder.encode(SigStructure);
  ...
  if (algName.startsWith('ES')) {
    ...
    if (typeof signer.sign === 'function') {
      ...
      // Direct key usage - noble requires prehash: true to hash the message
      // (noble's default is prehash: false, which assumes message is already hashed)
      const privKeyBytes = toUint8Array(signer.key);

      // prehash: true tells noble to hash ToBeSigned before signing (COSE standard)
      const signature = curve.sign(toUint8Array(ToBeSigned), privKeyBytes, { prehash: true });

      // For @noble/curves v2.x, signature is an object - extract compact bytes
      const sigBytes = signature.toCompactRawBytes
        ? signature.toCompactRawBytes()
        : signature;
      sig = Buffer.from(sigBytes);
    } else {
...
exports.create = async function (headers, payload, signers, options) {
  ...
  // CRITICAL: Use cborEncoder to avoid tag 259 on Maps
  if (bodyP.size === 0) {
    bodyP = Buffer.from([0xa0]); // CBOR empty map for Sign1 structure
  } else {
    bodyP = Buffer.from(cborEncoder.encode(bodyP));
  }
  ...
  if (Array.isArray(signers)) {
    ...
    signerP = (signerP.size === 0) ? EMPTY_BUFFER : Buffer.from(cborEncoder.encode(signerP));
    ...
    if (options.excludetag) {
      return Buffer.from(cborEncoder.encode(signed));
    } else {
      // Tag 98 (SignTag) needs to be manually constructed
      const encoded = cborEncoder.encode(signed);
...
async function doVerify (SigStructure, verifier, alg, sig) {
  ...
  if (algName.startsWith('ES')) {
    ...
    if (typeof verifier.verify === 'function') {
      ...
      // Accept high-S signatures from iOS Secure Enclave
      // iOS SecKeyCreateSignature produces s > n/2 (high-S) in ~50% of cases
      // This is valid per ECDSA spec but noble defaults to rejecting for malleability protection
      // prehash: true ensures the message is hashed before verification (ECDSA standard)
      const isValid = curve.verify(sigBytes, toUint8Array(ToBeSigned), pubKey, { lowS: false, prehash: true });
